...
namespace spa {
ResultTable CallsTransClause::Execute(
        KnowledgeBase *knowledge_base) const noexcept {
    switch (type_) {
        case kSynSyn: {
            auto [col_1, col_2] = knowledge_base->GetCallsTPairs();
        ...
        case kSynIdent: {
            auto result = knowledge_base->GetCallsT(
        ...
        case kIdentIdent: {
            auto result =
                    knowledge_base->ExistCallsT(first_ident_, second_ident_);
            return ResultTable(result);
        }
        default:
            assert(false);
