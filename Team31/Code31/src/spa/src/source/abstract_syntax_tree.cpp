    ...
    while (i < tokens_->size()) {
        switch (tokens_->at(i).type) {
            ...
            case kKeywordWhile:basic_entities_->whiles.emplace_back(line_number++);
                while (tokens->at(++i).type != kBraceL) {
                    switch (tokens->at(i).type) {
                        case kName:AddVariable(tokens->at(i).value);
                            break;
                        case kInteger:AddConstant(tokens->at(i).value);
                            break;
                        default:break;
                    }
                }
            ...
            case kKeywordIf:basic_entities_->ifs.emplace_back(line_number++);
                ...
                ++i;
                break;
            case kAssignEqual:AddVariable(tokens->at(i - 1).value);
                basic_entities_->assigns.emplace_back(line_number++);
                while (tokens->at(++i).type != kSemicolon) {
                    switch (tokens->at(i).type) {
                        case kName:AddVariable(tokens->at(i).value);
                            break;
                        case kInteger:AddConstant(tokens->at(i).value);
