...
class ModifiesRelationshipStore {
  public:
    ...
    [[nodiscard]] std::vector<int> GetStmtNo(int var_index)
            const;  // Gets a list of stmt that modifies var_index
    [[nodiscard]] int GetVarIndex(int stmt_no)
            const;  // Gets a var_index that is modified in given stmt_no

  private:
    std::vector<int> stmt_to_var_;  // stmtNo to var_index
    std::vector<std::vector<int>>
            var_to_stmtlst_;  // var_index to a list of stmt that modifies by
                              // var_index
};
}  // namespace spa
