...
enum PKBEntityType {
    kProc, kVar, kConst,
    kReadStmt, kPrintStmt, kCallStmt,
    kWhileStmt, kIfStmt, kAssignStmt,
...
class KnowledgeBase {
  public:
    ...
    * containing stmt# (only stmt at the outermost same level) inside the procedure.
    */
    virtual void SetIndex(Index<kProc> proc_index, Index<kStmtLst> stmtlst_index) = 0;

    /**
    * Links stmt# of while statement with its statement list's index
    * containing stmt# (only stmt at the outermost same level) inside while loop.
    */
    virtual void SetIndex(Index<kWhileStmt> stmt_no, Index<kStmtLst> stmtlst_index)= 0;

    /**
    * Links stmt# of if statement with its two statement lists' indices
    * containing stmt# (only stmt at the outermost same level) inside if clause and else clause respectively.
    */
    virtual void SetIndex(Index<kIfStmt> stmt_no, Index<kStmtLst> stmtlst_index1, Index<kStmtLst> stmtlst_index2) = 0;

    /**
    * Stores each statement list to its respective index.
    */
    virtual void SetLst(Index<kStmtLst> stmtlst_index, std::vector<STMT_NO> stmtlst) = 0;

    /**
    * Sets direct Uses relationships between print stmt# and its variable's index
    */
    ...
    * Procedure, variable and constant types are using the indices of their internal data structure.
    * The statement types are using stmt# directly as their indices.
    */
    virtual std::vector<std::string> IndexToName(std::vector<int> index_list, PKBEntityType et) = 0;
